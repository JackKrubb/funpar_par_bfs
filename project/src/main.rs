...
fn bfs(source: usize, graph: HashMap<usize, HashSet<usize>>) -> (HashMap<usize,usize>,HashMap<usize,usize>) {
    let mut frontier: HashSet<usize> = HashSet::from([source]); 
    let mut parent: HashMap<usize,usize> = HashMap::from([(source, source)]);
    let mut visited: HashSet<usize> = HashSet::from([source]); //Visited nodes
    let mut distance: HashMap<usize, usize> = HashMap::new(); //Hashmap of each vertices and its distances from the source 
    fn expand(frontier: HashSet<usize>, graph: &HashMap<usize, HashSet<usize>>, parent: HashMap<usize,usize>, visited: HashSet<usize>) -> (HashSet<usize>, HashMap<usize,usize>) {
    ...
    //         for (&key, &val) in distance.iter() {
    //             new_distance.insert(key, val+1);
    //         }
    //         iterate(&graph, new_frontier, new_parent,new_distance, d+1, visited)
    //     }
    // }
